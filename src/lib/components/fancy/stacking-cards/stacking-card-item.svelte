...
<script lang="ts">
	...
	let scaleTo = $derived(
		1 - (safeTotalCards - index) * stackingCardsContext.scaleMultiplier
	);
	...
	let scale = useTransform(() => {
		...
		const normalizedProgress =
			rangeScaleStart === 1 ? Number(progress >= 1) : (clampedProgress - rangeScaleStart) / (1 - rangeScaleStart);
...
<div
	class={cn('sticky top-0 h-full', className)}
	{...props}
>
	<motion.div
		class="origin-top relative h-full"
		style={{ top, scale }}
	>
		{@render children?.()}
	</motion.div>
