<script lang="ts">
	import type { Snippet } from "svelte";
	import * as Frame from "$lib/components/ui/frame";
	import { Button } from "$lib/components/ui/button";
	import { cn } from "$lib/utils";
	import { mode } from "mode-watcher";

	interface FancyShowcaseCardProps {
		title: string;
		href: string;
		children?: Snippet;
		class?: string;
		previewClass?: string;
	}

	let {
		title,
		href,
		children,
		class: className,
		previewClass,
	}: FancyShowcaseCardProps = $props();

	let retry = $state(0);
	let updateRetry = () => {
		// random retry logic to trigger remount
		if (retry > 5) {
			retry = 0;
		} else {
			retry++;
		}
	};
</script>

<article class={cn("group h-full", className)}>
	<Frame.Root class="w-full">
		<Frame.Header class="flex flex-row items-center justify-between px-2 py-1.5">
			<Frame.Title class="font-figtree font-medium">{title}</Frame.Title>
			<div class="hidden items-center gap-1 md:flex">
				<Button
					{href}
					class="font-figtree rounded-full"
					variant={mode.current === "light" ? "outline" : "secondary"}
					size="sm">Visit</Button
				>
				<Button
					onclick={updateRetry}
					class="group cursor-pointer rounded-full"
					variant={mode.current === "light" ? "outline" : "secondary"}
					size="icon-sm"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						width="32"
						height="32"
						viewBox="0 0 24 24"
						class="size-5"
						><path
							fill="currentColor"
							d="M11.896 18a.75.75 0 0 1-.75.75c-3.792 0-6.896-3.005-6.896-6.75s3.104-6.75 6.896-6.75c3.105 0 5.749 2.015 6.605 4.801l.603-1.02a.75.75 0 0 1 1.292.763l-1.63 2.755a.75.75 0 0 1-1.014.272L14.18 11.23a.75.75 0 1 1 .737-1.307l1.472.83c-.574-2.288-2.691-4.003-5.242-4.003C8.149 6.75 5.75 9.117 5.75 12s2.399 5.25 5.396 5.25a.75.75 0 0 1 .75.75"
						/></svg
					>
				</Button>
			</div>
		</Frame.Header>
		<Frame.Panel class="h-full min-h-80 w-full rounded-xl overflow-hidden p-0">
			{#key retry}
				{@render children?.()}
			{/key}
		</Frame.Panel>
	</Frame.Root>
</article>
