<script lang="ts" module>
	...
	export const richButtonVariants = tv({
		base: "cursor-pointer inline-flex items-center justify-center gap-2 whitespace-nowrap font-medium focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 inset-shadow-2xs inset-shadow-white/25 bg-linear-to-b dark:bg-linear-to-t border border-zinc-950/35 shadow-md shadow-zinc-950/20 ring-0 transition-[filter] duration-200 hover:brightness-110 active:brightness-95 dark:border-0 dark:border-zinc-950/50",
		variants: {
			size: {
	...
	export type RichButtonColor = keyof typeof colorMap;
	export type RichButtonSize = VariantProps<typeof richButtonVariants>["size"];
	export { colorMap, textShadowMap };
</script>
...
<script lang="ts">
	...
	let {
		ref = $bindable(null),
		class: className,
		color = "default" as RichButtonColor,
		size = "default" as RichButtonSize,
		children,
		...restProps
	...
	const textColor = $derived(color === "default" ? "text-white dark:text-zinc-900" : "text-white");
	const colorClasses = $derived(colorMap[color]);
	const textShadowClasses = $derived(textShadowMap[color]);
</script>

<button
	bind:this={ref}
	data-slot="rich-button"
	class={cn(richButtonVariants({ size }), colorClasses, textColor, className)}
	{...restProps}
>
	<span class={cn("relative inline-flex items-center gap-2", textShadowClasses)}>
		{@render children?.()}
	</span>
</button>
