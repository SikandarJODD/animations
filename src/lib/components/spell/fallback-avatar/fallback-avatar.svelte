...
void main(){
  ...
  gl_FragColor=vec4(col,1);
	...
	let timeRef = $state(0);
	let uniforms: Uniforms | null = $state(null);

	function getUniforms(): Uniforms {
		if (!uniforms) {
			uniforms = computeUniforms(name);
		}
		return uniforms;
	}

	$effect(() => {
		// Reset uniforms when name changes
		name;
		uniforms = null;
	});
	...
	onMount(() => {
		...
		const animate = (timestamp: number) => {
			...
			if (startTime === null) startTime = timestamp - timeRef * 1000;
		...
		const onLeave = () => {
			...
			renderToCanvas(canvas, getUniforms(), timeRef);
		};
