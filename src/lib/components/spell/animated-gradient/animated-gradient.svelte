<script lang="ts">
	...
	let {
		config = { preset: 'Prism' } as GradientConfig,
		noise,
		radius = '0px',
		style = '',
		class: className = '',
	...
	function getParams(cfg: GradientConfig): PresetParams {
...
<div
	bind:this={container}
	class={className}
	style="position: absolute; inset: 0; z-index: -1; border-radius: {radius}; overflow: hidden; {style}"
>
	<canvas
		bind:this={canvas}
	...
	{#if noise && noise.opacity > 0}
		<div
			style="position: absolute; inset: 0; background-image: url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAADAAAAAwBAMAAAClLOS0AAAAElBMVEUAAAAAAAAAAAAAAAAAAAAAAADgKxmiAAAABnRSTlMCCgkGBAVJOAVJAAAASklEQVQ4y2NgGAWjYBSMglEwCgY/YGRgZBQUYmJiZGQEkYwMjIyMgoKCjIyMIJKBgRFIMjIyAklGRkYGRkFBYEcwMDIyMjAOUQAA1I4HwVwZAkYAAAAASUVORK5CYII='); background-size: {(noise.scale ?? 1) * 200}px; background-repeat: repeat; opacity: {noise.opacity / 2}; pointer-events: none;"
		></div>
	{/if}
</div>
