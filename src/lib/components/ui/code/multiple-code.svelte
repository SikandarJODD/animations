...
<div class="flex overflow-auto rounded-md border">
  <div
    class="hidden min-w-66 border-r overflow-auto rounded-tl-md bg-neutral-50 font-mono text-black [--color-background:var(--color-zinc-900)] [--color-foreground:white] [--color-muted:var(--color-zinc-800)] sm:block dark:bg-zinc-900/25 dark:text-white"
  >
    <div>
      <div
      ...
      {#each code as item, index}
        <Button
          class={cn(
            `flex w-full items-center justify-start gap-1.5 rounded-none pl-6! text-xs hover:bg-neutral-200/70 hover:dark:bg-zinc-900 border-l-2 border-transparent [&_svg:not([class*='size-'])]:size-3`,
            selectedIndex === index &&
              "bg-neutral-200/40 dark:bg-zinc-800/50 border-l-2 border-muted-foreground",
          )}
          variant="ghost"
  ...
  <div
    class="relative max-h-[550px] min-h-[32rem] w-full sm:w-full overflow-auto border-none"
  >
    <!-- <Code.Overflow> -->
    <Code.Root
